...
import static frc.robot.utils.PoseBuilder.CoordinateSystem.*;
...
public class PoseBuilder implements Supplier<Pose2d> {
	...
	public static PoseBuilder getAprilTagFacingPose(IntSupplier tagID) {
		...
		return PoseBuilder.getAprilTagPose(tagID)
			.withRotation(CardinalDirections.DOWN.getDegrees())
			.withTranslation(ROBOT_RELATIVE, RobotDimensions.ROBOT_LENGTH.div(2), CardinalDirections.DOWN.getDegrees());
	...
	public static PoseBuilder getHubShootingPose(
		...
		return PoseBuilder.fromPose(new Pose2d(VirtualField.getHubCenterPoint(), Rotation2d.kZero))
			.withHeading(FIELD_RELATIVE, CardinalDirections.RIGHT.getDegrees())
			.withRotation(shootingAngle)
			.withTranslation(ROBOT_RELATIVE, shootingRadius, CardinalDirections.RIGHT.getDegrees());
